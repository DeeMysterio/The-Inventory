...
def register():
    if current_user.is_authenticated:
        return redirect(url_for('products.show_report'))
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash_message = 'Your account has been created!'
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            return redirect(next_page) if next_page else redirect(url_for('products.show_report'))
        else:
